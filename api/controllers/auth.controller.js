...
export const signup = async (req, res, next) => {
  ...
  //  implimenting error if we got for same eamil and username

  try {
    await newuser.save();    // save newuser details in data base
    res.status(201).json("user created sussfully");
  } catch (error) {
    next(error);
  }
};
